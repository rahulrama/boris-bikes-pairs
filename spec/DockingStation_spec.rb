require 'DockingStation'

require 'bike'

describe DockingStation do

    it 'expects released to be working' do
        bike = Bike.new
        subject.dock(bike)
    expect(subject.release_bike).to be_working

    end

    it 'expects a bike to be docked' do
       bikes = Bike.new
	expect(subject.dock(bikes)).to eq [bikes]
    end

   it 'should be able to successfully docking bike' do
	bikes = Bike.new
	subject.dock(bikes)
	expect(subject.bikes).to eq [bikes]
   end

   it 'If empty raise error if there is an teempt to release bike' do
   expect{subject.release_bike}.to raise_error 'No bikes available'
   end

   it 'allows 45 bikes to be docked when 45 is the capacity' do
   docking_station = DockingStation.new(45)
   bike = Bike.new
   45.times{docking_station.dock(bike)}
   expect(docking_station.bikes.length).to eq 45
  end


   describe '#dock' do
       it 'raises an error when exceding DEFAULT_CAPACITY when no custom caacity is given' do
           DockingStation::DEFAULT_CAPACITY.times { subject.dock Bike.new }
           expect { subject.dock Bike.new }.to raise_error 'Dock already full'
           end

        it 'raises an error when exceding Capacity when custom caacity is given' do
          docking_station = DockingStation.new(75)
          bike = Bike.new
          75.times{docking_station.dock(bike)}
          expect{docking_station.dock(bike)}.to raise_error 'Dock already full'
          end


       end

end

